...
mod tests {
    ...
    #[test]
    fn migrating_version_11_accepts_nothing() {
        use crate::state::tests::{downgrade_to_v11, version};
        let mut case = Case::passed(&ACCEPTED, &SCOPE, &EDITS);
        let path = case.root().join(STATE_DIR).join(STATE_DB);
        let tables = |path: &Path| {
        ...
        let scratch = case.root().join(STATE_DIR).join("scratch.db");
        drop(std::mem::replace(
            &mut case.fx.store,
            Store::open(&scratch).unwrap(),
        ));
        ...
        for table in ["acceptances", "acceptance_sources", "acceptance_phases"] {
            ...
            assert_eq!(count, [[Value::Integer(0)]], "{table}");
        }
        assert_eq!(case.phase(), None);
        assert_eq!(case.generation_state(), GenerationState::Active);
        // A pass from before is accepted afresh, through Block 13 only.
        completed(case.accept());
    }
}
