...
mod tests {
    ...
    #[test]
    fn migrating_version_8_never_strengthens_a_capture() {
        use crate::state::SCHEMA_VERSION;
        use crate::state::tests::{downgrade_to_v8, rows_besides, version};
        ...
        let candidate = |store: &mut Store| {
            ...
            assert_eq!(outcome.unwrap(), ExecutionOutcome::Candidate);
            retire(store, generation);
            (generation, capture(store, generation))
        };
        let (kept, kept_capture) = candidate(&mut store);
        let (doubted, doubted_capture) = candidate(&mut store);
        let (unknown, ..) = attempted(&mut store, &["src/b.rs"], &[]);
        let (intended_only, ..) = intended(&mut store, &["src/c.rs"], &[]);
        let (forged, _, invocation) = attempted(&mut store, &["src/d.rs"], &[]);
        store.finish_invocation(invocation, &succeeded()).unwrap();
        let plan = store.create_plan(&objective("elsewhere")).unwrap();
        let planner = store
            .create_agent(Role::Planner, AgentScope::Plan(plan))
        ...
        // What version 8 disregarded for changes within authority: another
        // agent's action, in flight while `doubted` executed.
        Connection::open(&path)
            .unwrap()
            .execute(
                "INSERT INTO journal (agent_id, action, parameters, state, intended_at,
                   attempted_at)
                 SELECT ?1, 'elsewhere', '{}', 'attempted', started_at, started_at
                 FROM executions WHERE generation_id = ?2",
                params![planner, doubted],
            )
            .unwrap();
        // Version 8 let raw SQL assert a candidate its journal never saw.
        let promoted = Connection::open(&path)
            .unwrap()
            .execute(
                "UPDATE executions SET outcome = 'candidate', reported = 'succeeded',
                   claimed = '[]', head_after = head, captured_at = 1
        ...
        // Not carried over: nothing reconciled it.
        assert!(matches!(
            status(&store, forged),
            ExecutionStatus::OutcomeUnknown {
                invocation: Some(_)
        ...
        let captures: i64 = store
            .conn
            .query_row("SELECT count(*) FROM execution_captures", [], |r| r.get(0))
            .unwrap();
        assert_eq!(captures, 2);
        assert_eq!(rows_besides(&path, &[]), before, "nothing else changed");
        assert_eq!(capture(&store, kept), kept_capture);
        let migrated = capture(&store, doubted);
        assert_eq!(
            (migrated.outcome, migrated.attribution),
            (
                ExecutionOutcome::Unattributable,
                Some(Attribution::Concurrent)
            )
        );
        let unchanged = Capture {
            outcome: ExecutionOutcome::Candidate,
            attribution: None,
            ..migrated
        };
        assert_eq!(unchanged, doubted_capture, "only weakened");
        assert!(matches!(
        ...
        assert_eq!(status(&store, intended_only), ExecutionStatus::Intended);
        // Migrated history is held as a fresh store holds it.
        let message = store
            .conn
            .execute("UPDATE execution_captures SET outcome = 'candidate'", [])
            .unwrap_err()
            .to_string();
        assert!(message.contains("history is immutable"), "{message}");
    }
    ...
    #[test]
    fn migrating_version_9_keeps_every_capture_uninstalled() {
        use crate::state::SCHEMA_VERSION;
        use crate::state::tests::{downgrade_to_v9, rows_besides, version};
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("v9.db");
        let mut store = Store::open(&path).unwrap();
        let (kept, _) = candidate(&mut store);
        let kept_capture = capture(&store, kept);
        let (unknown, ..) = attempted(&mut store, &["src/b.rs"], &[]);
        drop(store);
        ...
        assert_eq!(capture(&store, kept), kept_capture);
        assert_eq!(kept_capture.install, Install::NotAttempted);
        assert!(matches!(
            status(&store, unknown),
            ExecutionStatus::OutcomeUnknown { .. }
        ));
